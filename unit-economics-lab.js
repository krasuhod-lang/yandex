...
(function () {
  ...
  // в этот Set (сериализуется в localStorage). applyMode использует его,
  // чтобы НЕ перетирать правки жёстко зашитыми значениями To-Be — тогда
  // изменения ползунков (в том числе по сегментам) сразу видны в юнит-экономике.
  function loadEdited() {
    try {
