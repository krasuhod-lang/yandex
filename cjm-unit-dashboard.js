(function(){
  ...
  var segments=[
    {
      ...
      cac:780,cpa:0,ltv:1800,payback:5,share:0.30,
    ...
    },
    {
      ...
      cac:380,cpa:3000,ltv:4500,payback:3,share:0.25,
    ...
    },
    {
      ...
      cac:870,cpa:2200,ltv:2400,payback:5,share:0.25,
    ...
    },
    {
      ...
      cac:600,cpa:0,ltv:3500,payback:3,share:0.05,
    ...
    },
    {
      ...
      source:'SEO + контекст + ремаркетинг + соц. сети',
      router:'API NOT_FOUND (после поверхностной проверки) → Пробив чеккером по номеру телефона → витрина банковских продуктов или 5 МФО',
      ...
      cac:920,cpa:3500,ltv:3500,payback:6,share:0.15,
  ...
  function renderHero(){
    ...
    if(isMatrixView()){
      ...
      lead.textContent='Сравнение 5 сегментов рядом: воронки на 10 000 пользователей, юнит-экономика, каналы привлечения и LTV/CAC. Сегмент-лидер по выручке отмечен 🏆. Чтобы перейти к деталям конкретного сегмента — нажмите на его таб сверху.';
    }else{
      var s=currentSegment();
      eyebrow.textContent='Раздел сайта · CJM & Юнит-экономика — сегмент: '+s.name;
      title.textContent=s.label;
      lead.textContent=s.pains+' Внутри 4 вкладок-переключателей: воронка на 10 000 пользователей с ручными показателями, путь клиента (описание сегмента + блок-схема только этого сегмента), пример расчёта выручки и юнит-экономика.';
    }
  }
  ...
  function renderRoutingDiagram(){
    ...
    var allBranches=[
      ...
      { key:'noncore', color:'var(--violet)', name:'Непрофильный клиент',
        seg:{t:'Непрофильный клиент',s:'Фиолетовый · NOT_FOUND'},
